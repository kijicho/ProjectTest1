...
def testDefinitionFind(FileData, tempTxt, p):
    findFlag = 0
    blockStartFlag = 0
    blockCount = 0
...
nvmKeysName = dict.keys(dictDemInform)
...
for mVariablesBlockName in nvmKeysName:
    p.append(re.compile('^u.+' + mVariablesBlockName.replace('FeeBlock_','') + '.+;'))
    ...
    if len(files) == 0:
        ...
        for file in files:
            if file.endswith('.c'):     #확장자가 '.c'인 파일만 추출
                ...
                try:
                    f = open(path + "\\" + file, 'r', encoding='cp949')
                    testDefinitionFind(f,tempExtractTxt, p)
