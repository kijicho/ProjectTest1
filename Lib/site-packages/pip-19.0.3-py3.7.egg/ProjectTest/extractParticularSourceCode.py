...
def testFunction(FileData, tempTxt):
    findFlag = 0
    blockStartFlag = 0
    blockCount = 0
    ...
    for line in f.readlines():
        if findFlag == 0:
            ...
            if (tempLine.find('//') == 0) or (tempLine.find('*') == 0) or (commentStartFlag == 1):
                continue
        if findFlag == 0:
            if (p[0].match(line) != None or p[1].match(line) != None or p[2].match(line) != None) and line.find(
                    ';') == -1:
                print(line)  # 라인 출력
...
p.append(re.compile('.+_Set_m_.*\('))

# 2. Job Finished를 추출하기 위한 필터
p.append(re.compile('.+_JobFinished.*'))
